...
const SUITS = {
  ...
  S: { name: 'spade', color: '#c4b7eb', darkColor: '#a99bd4' }
};
...
function createCardSVG(rank, suit) {
  ...
  svg += `<defs>
    <linearGradient id="cardBg${rank}${suit}" x1="0%" y1="0%" x2="100%" y2="100%">
      ...
      <stop offset="100%" style="stop-color:#f8f6ff"/>
    </linearGradient>
...
function createCardBackSVG() {
  ...
  svg += `<defs>
    <linearGradient id="backGrad" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" style="stop-color:#2d2640"/>
      <stop offset="50%" style="stop-color:#1a1625"/>
      <stop offset="100%" style="stop-color:#2d2640"/>
    </linearGradient>
    <pattern id="dots" x="0" y="0" width="8" height="8" patternUnits="userSpaceOnUse">
      <circle cx="4" cy="4" r="1.5" fill="#c4a7e7" opacity="0.3"/>
    </pattern>
  </defs>`;
  ...
  // Border decorations
  svg += `<rect x="4" y="4" width="${width - 8}" height="${height - 8}" fill="none" stroke="#c4a7e7" stroke-width="1.5" rx="6" opacity="0.4"/>`;
  svg += `<rect x="8" y="8" width="${width - 16}" height="${height - 16}" fill="none" stroke="#c4a7e7" stroke-width="1" rx="4" stroke-dasharray="4 2" opacity="0.3"/>`;

  // Center decoration - cute star
  const cx = width / 2;
  const cy = height / 2;
  svg += `<polygon points="${cx},${cy - 8} ${cx + 3},${cy - 2} ${cx + 9},${cy - 2} ${cx + 4},${cy + 2} ${cx + 6},${cy + 8} ${cx},${cy + 5} ${cx - 6},${cy + 8} ${cx - 4},${cy + 2} ${cx - 9},${cy - 2} ${cx - 3},${cy - 2}" fill="#c4a7e7" opacity="0.5"/>`;
